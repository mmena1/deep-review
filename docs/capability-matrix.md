...
| Static validator | `code-reviewer-validator-static`, Sol high; `exec` is available, so no-write/no-probe confinement is instruction-enforced | `deep_review_validator_static`, Sol high, read-only sandbox |
